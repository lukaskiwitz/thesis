...
saving_string = r"/home/brunner/Documents/Current work/2024_03_15/boxplot_static_std.pdf"
offset = 0
...
for frac in ODE_fracs:
    ...
    for idx, rep in enumerate(np.sort(ODE_frac_df[replicat].unique())):
        ...
        frac_act = tmp_df["IL-2_surf_c"].std()
...
for frac in fracs:
    ...
    for rep in frac_df["replicat_index"].unique():
        ...
        act_list_2.append(rep_df["IL-2_surf_c"].std())
...
# plt.plot(np.array(ODE_fracs) * 100, np.mean(ODE_boxplot_data, axis=1), color=color2, linewidth=0.9)
