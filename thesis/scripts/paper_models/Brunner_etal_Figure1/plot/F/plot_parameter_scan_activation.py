...
path = "/extra2/brunner/paper_models/boxed_static/parameter_scan_redo/"

fig_path = "/home/brunner/Documents/Current work/2023_11_03/"  + "Fig1C_act_short.pdf"
    ...
    for value in plotted_sv:
        ...
        for rep in name_df["replicat_index"].unique():
            if scan_measure == "pSTAT5":
                ...
                name_df["pSTAT5"] = name_df["IL-2_surf_c"] ** 3 / (
                        (EC50_calculation(E_max=125e-12, E_min=0, k=860, N=1.5, R=name_df["IL-2_R"]) * 1e12) ** 3 +
                        name_df["IL-2_surf_c"] ** 3).values
                try:
                    standard = len(name_df.loc[(name_df["replicat_index"] == rep) & (np.abs(name_df["scan_value"] - 1) < 1e-12) & (
                                name_df["pSTAT5"] > 0.5)]) / \
                               len(name_df.loc[(name_df["replicat_index"] == rep) & (np.abs(name_df["scan_value"] - 1) < 1e-12)])
                    a_mean = len(name_df.loc[(name_df["replicat_index"] == rep) & (np.abs(name_df["scan_value"] - value) < 1e-12) & (
                            name_df["pSTAT5"] > 0.5)]) / \
                             len(name_df.loc[(name_df["replicat_index"] == rep) & (np.abs(name_df["scan_value"] - value) < 1e-12)])

                    rep_standards.append(standard * 100)
                    rep_as.append(a_mean * 100)
...
if scan_measure == "surf. c.":
    ...
    for e,entry in enumerate(bars):
        for v, value in enumerate(entry):
            # ax.bar(x=x_reshaped[e,v], height=value if value > 2e-0 else 2e-0, color="black", alpha=alphas[v], yerr=error_reshaped[e,v], width=width, bottom=0,
            #        linewidth=0.3, error_kw=dict(lw=width/2, capsize=0, capthick=0), edgecolor="black")
            ax.bar(x=x_reshaped[e,v], height=value if value > 2e-0 else 2e-0, color=color, alpha=alphas[v], yerr=error_reshaped[e,v], width=width, bottom=0,
                   linewidth=0.0, error_kw=dict(lw=width/2, capsize=0, capthick=0), edgecolor="black")
