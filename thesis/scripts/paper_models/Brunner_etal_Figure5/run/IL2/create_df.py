...
for path in path_list:
    ...
    try:
        ...
        try:
            ...
            print("Saving the cell_df to hdf failed, falling back to pickling...")
            plotter.cell_df.to_pickle(os.path.join(full_folder_path, "cell_df.pkl"))
        try:
            plotter.global_df.to_hdf(os.path.join(full_folder_path, "global_df.h5"), key="df", mode="w")
        except Exception as e:
            print(e)
            plotter.global_df.to_pickle(os.path.join(full_folder_path, "global_df.pkl"))
...
try:
    ...
    print("Saving the cell_df to hdf failed, falling back to pickling...")
    df.to_pickle(os.path.join(SS_FOLDER_PATH, "cell_df.pkl"))
...
try:
    ...
    print("Saving the global_df to hdf failed, falling back to pickling...")
    df.to_pickle(os.path.join(SS_FOLDER_PATH, "global_df.pkl"))
