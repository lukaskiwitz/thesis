...
saving_string = "/{extra}/{u}/paper_models/kinetics/{mn}/Tsec_c_R_std_feedback_analysis_1.svg".format(u=user, mn=model_name, extra = hdd)
...
ylimR = (1e2, 3e6) # ylim for receptors
...
fraction_of_cells_displayed = 1/4
all_cell_ids = cell_df["id"].unique()
stay_ids = np.random.choice(all_cell_ids, int(len(all_cell_ids) * fraction_of_cells_displayed))
cell_df = cell_df[cell_df["id"].isin(stay_ids)]
