...
def get_conditioned_scan_sample(pool, naive, sec, treg, parameter_dict, scan_value_dict):
    ...
    KD_t = PhysicalParameterTemplate(PhysicalParameter("KD", 7.437e-3, to_sim=1 / get_cc(-6)))
    k_off_t = PhysicalParameterTemplate(PhysicalParameter("k_off",  2.3e-4 , to_sim=1 / 60**2))
    ...
    if "KD" in value_dict.keys():
        KD = value_dict["KD"]
        Th_parameter_set.update(ParameterCollection("IL-2", [KD_t(KD)], field_quantity="il2"))
    if "k_off" in value_dict.keys():
        k_off = value_dict["k_off"]
        Th_parameter_set.update(ParameterCollection("IL-2", [k_off_t(k_off)], field_quantity="il2"))
    ...
    sample = ScanSample(
        ...
        [
            ...
            naive.get_updated(Th_parameter_set),
...
def compute_samples_points(scan_samples, scenario, path, time_range, ext_cache=""):
    ...
    uS = updateState(0, 0, scenario.global_parameters.get_collection("geometry").get_as_dictionary(), scenario.parameter_pool, [], [], [], offset=0)
    ...
    state_manager.run(ext_cache=ext_cache, model_names=["pde_model"], number_of_replicats=40)
...
def extract_scan_axis_from_dictionary(scan_kwargs):
    ...
    for k, v in scan_kwargs.items():
        if isinstance(v, List):
            scan_scale = np.logspace(v[0], v[1], int(v[2]))
            scan_value_name = k
            break
