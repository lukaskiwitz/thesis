...
t_k_on = parameter_pool.get_template("k_on")
t_KD = parameter_pool.get_template("KD")
...
KD = ScannableParameter(t_KD(0), lambda x, v: v)
...
# scan_space = np.linspace(0.01,40,20) * 1e-3 # post is in nM
scan_space = np.logspace(-2,2, 21, base=2) * 7.4 * 1e-3 # post is in nM
# scan_space = [0.01,0.1,0.5]
# kds = [0.001,0.01,0.1]

KD_def = ScanDefintion(KD, "IL-2", scan_space, ScanType.ENTITY, field_quantity="il2", entity_type=Th)
...
scan_container.add_single_parameter_scan([KD_def], scan_name="KD")
...
def pre_replicat(sc, time_index, replicat_index, t, T):
    ...
    uS.replicat_index = replicat_index
    uS.step(sc)
