...
saving_string = r"/home/brunner/Documents/Current work/2022_03_04/" + "cv_{m}_static_{sv}_{bc}".format(m = "spatial_ODE_smoothed", sv=scan_variables[0], bc = bc) + ".pdf"
...
for m,model in enumerate(models):
    for sv,scan_variable in enumerate(scan_variables):
        ...
        if interpolate == True:
            new_x, spl, ax = my_interpolation(x, np.array(cv), smoothing_factor=sfs[sv], label=str(model), color=model_colors[m][sv], linestyle = model_styles[m], alpha=model_alphas[m])
        else:
            plt.plot(x, cv, label=str(model), color=model_colors[m][sv], linestyle = model_styles[m], alpha=model_alphas[m])
