...
scan_space = np.logspace(-2,2, 21, base=2) * 0.05
scan_space = scan_space[scan_space < 1]
scan_space = np.concatenate([[0.001], scan_space])
...
scan_container.add_single_parameter_scan([Tsec_def], scan_name="Tsec_fraction")
...
def post_replicat(sc, time_index, replicat_index, t, T):
    ...
    message("#####")
    message("#####")
    message("SD in M: " + str(np.std(list)))
    message("#####")
