...
saving_string =r"/home/brunner/Documents/Current work/2021_10_15/" + "static_R_test" + ".svg"
...
offset = 0
# which cell types to plot in which color
cell_types = ["Th"]
cell_colors = ["black"]
...
for c, cell_type in enumerate(cell_types):
    ...
    sns.lineplot(np.sort(tmp_df[scan_variable].unique()), surf_c,
                 color=cell_colors[c], ci=0, label="linear " + cell_type)
...
for c, cell_type in enumerate(cell_types):
    ...
    plt.fill_between(np.sort(bars_df[scan_variable].unique()),
                     ...
                     np.clip(np.array(mean) + np.array(ste), 0, None), alpha=0.15, color=cell_colors[c])
...
# first_line = mlines.Line2D([], [], color=cell_colors[0])
# second_line = mlines.Line2D([], [], color=cell_colors[1])
#
# first_line_dashed = mlines.Line2D([], [], color=cell_colors[0], linestyle="dashed")
# second_line_dashed = mlines.Line2D([], [], color=cell_colors[1], linestyle="dashed")
#
#
...
if plot_legend == True:
    ...
    labels = []
    for c, cell_type in enumerate(cell_types):
        handles.append(mlines.Line2D([], [], color=cell_colors[c]))
