...
try:
    ...
    hist_df = pd.read_hdf(path + "hist_df_c_test_" + str(threshold) + ".h5", mode="r")
    ...
    for time in time_points: #cell_df["time"].unique():
        ...
        for il2cells in cell_df.loc[(cell_df["type_name"] == "changed") & (cell_df["time"] == 0), "id"]: #small_cell_sample: #
            ...
            # put into df
            hist_df[str(time)][str(il2cells)] = [np.array(data).astype(np.float), np.array(distance).astype(np.float)] #cell_df.loc[cell_df["time"] == time, "distance"]
            normalising_df[str(time)][str(il2cells)] = [cell_df_timed["IL-2_surf_c"].mul(1e3).to_numpy().astype(np.float), cell_df_timed["distance"]]
    # save to file
    hist_df.to_hdf(path + "hist_df_c_test_" + str(threshold) + ".h5",key="df", mode="w")
