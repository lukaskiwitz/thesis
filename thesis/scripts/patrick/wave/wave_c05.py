...
try:
    ...
    hist_df = pd.read_hdf(path + "hist_df_c_05_large_" + str(threshold) + ".h5", mode="r")
    ...
    # save to file
    hist_df.to_hdf(path + "hist_df_c_05_large_" + str(threshold) + ".h5",key="df", mode="w")
...
time_list = ["0.0", "5.0", "50.0"]
# time_list.reverse()
random_samples = random.sample(range(len(hist_df["0.0"][0][1])),100)
...
for plot_time_point in time_list:
    ...
    for cell in hist_df[plot_time_point]:
        for i in random_samples: # range(len(cell[1])):
            bin_index = 0
...
#     plt.title("Change over time")
    ...
    plt.ylabel("Mean surface c. in pM")
    # plt.ylim((0,380))
    plt.xticks(bins[:14]-10, [int(x/20) for x in bins][:14])
    # plt.xlim((0,240))
fig.savefig("/home/brunner/Documents/Current work/26062020/" + "wave_c05_v1" + ".png", bbox_inches='tight')
# plt.show()