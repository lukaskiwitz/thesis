...
time_points = cell_df["time"].unique()#[0., 10., 19.]
...
for plot_time_point in time_list:
    ...
    for i in range(len(matrix)):
        ...
        # small = shifted[1:,:-1]
        # first = np.append(small, [shifted[1:,-1]], 0)
...
ax1 = sns.lineplot(center[:len(hist_data_array[0])], hist_data_array[0], label=str(float(time_list[0])*10)+"h", legend=False)
...
fig.add_subplot(a_x, a_y, 2)
ax4 = sns.lineplot(center[:len(c_data_array[0])], c_data_array[0], label=str(float(time_list[0])*10)+"h", legend=False)
...
ax7 = sns.lineplot(center[:len(conv_data_array[0])], conv_data_array[0], label=str(float(time_list[0])*10)+"h", legend=False)
...
ax10 = sns.lineplot(center[:len(auto_corr_data_array[0])], auto_corr_data_array[0], label=str(float(time_list[0])*10)+"h", legend=False)
