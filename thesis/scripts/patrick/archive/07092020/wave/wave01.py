...
# print(len(cell_df["id_id"].unique()))
#
# exit()
...
try:
    ...
    hist_df = pd.read_hdf(path + "hist_df_0.1_large_" + str(threshold) + ".h5", mode="r")
    ...
    hist_df.to_hdf(path + "hist_df_0.1_large_" + str(threshold) + ".h5",key="df", mode="w")
...
for plot_time_point in time_list:
    ...
    plt.plot(center[:13], hist_norm[:13], "-o", label=str(float(plot_time_point)*10)+"h")
