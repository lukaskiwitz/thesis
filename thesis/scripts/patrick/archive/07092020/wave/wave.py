...
threshold = 15000 #if less than x Receptors

try:
    # print("loading name changed to test")
    hist_df = pd.read_hdf(path + "hist_df_large_" + str(threshold) + ".h5", mode="r")
    normalising_df = pd.read_hdf(path + "normalising_df_large" + ".h5", mode="r")
    ...
    for time in time_points: #cell_df["time"].unique():
        ...
        for il2cells in cell_df.loc[(cell_df["type_name"] == "changed") & (cell_df["time"] == 0), "id"]: #small_cell_sample: #
            ...
            hist_df[str(time)][str(il2cells)] = np.array(data).astype(np.float).copy() #cell_df.loc[cell_df["time"] == time, "distance"]
            normalising_df[str(time)][str(il2cells)] = cell_df_timed["distance"].to_numpy().astype(np.float).copy()

    hist_df.to_hdf(path + "hist_df_2_large_" + str(threshold) + ".h5",key="df", mode="w")
    hist_df[str(0.)].to_hdf(path + "hist_df_large_0_" + str(threshold) + ".h5",key="df", mode="w")
    hist_df[str(5.)].to_hdf(path + "hist_df_large_5_" + str(threshold) + ".h5", key="df", mode="w")
    hist_df[str(50.)].to_hdf(path + "hist_df_large_50_" + str(threshold) + ".h5", key="df", mode="w")
    normalising_df.to_hdf(path + "normalising_df_large" + ".h5", key="df", mode="w")
