...
def updateState(sc, t):
    ...
    tmp_fraction = 0.25 #sc.entity_list[0].p.get_physical_parameter("fraction", "IL-2").get_in_post_unit()
    ...
    for i, e in enumerate(sc.entity_list):
        if no_of_secs != 0 and e.change_type == "changed":
            ...
            E = 5000 * N_A ** -1 * 1e9 #e.p.get_physical_parameter("R", "IL-2").get_in_sim_unit()
            var = sc.p.get_physical_parameter("sec_sigma", "IL-2").get_in_sim_unit()
            if var == 0:
                new_R = E
                e.p.get_physical_parameter("R", "IL-2").set_in_sim_unit(new_R)
                e.p.add_parameter_with_collection(t_R_start(new_R, in_sim=False))
            else:
                tmp_sigma = np.sqrt(np.log(var ** 2 / E ** 2 + 1))
                mean = np.log(E) - 1 / 2 * tmp_sigma ** 2

                new_R = np.random.lognormal(mean, tmp_sigma)
                e.p.get_physical_parameter("R", "IL-2").set_in_sim_unit(new_R)
                e.p.add_parameter_with_collection(t_R_start(new_R, in_sim=False))
        elif e.change_type == "Treg":
            E = 27000 * N_A ** -1 * 1e9 # e.p.get_physical_parameter("R", "IL-2").get_in_sim_unit()
            var = sc.p.get_physical_parameter("Treg_sigma", "IL-2").get_in_sim_unit()
            if var == 0:
                new_R = E
                e.p.get_physical_parameter("R", "IL-2").set_in_sim_unit(new_R)
                e.p.add_parameter_with_collection(t_R_start(new_R, in_sim=False))
...
for j in range(1):
    ...
    t_Treg_sigma = templates["Treg_sigma"]
    t_sec_sigma = templates["sec_sigma"]
    ...
    Treg_sigma = ScannablePhysicalParameter(t_Treg_sigma(0.0), lambda x, v: v)
    sec_sigma = ScannablePhysicalParameter(t_sec_sigma(0.0), lambda x, v: v)


    q_distribution_array = np.array([])
    Treg_distribution_array = np.array([])

    for Treg_sig in np.linspace(000.1, 27000.0, 10): #np.logspace(-1,1,3):
        ...
        #         c0_value = 8e-12
            ...
            sim_parameters = [
                ParameterCollection("IL-2", [Treg_sigma(Treg_sig)], field_quantity="il2"),
                ParameterCollection("IL-2", [Treg_fraction(0.25)], field_quantity="il2"),
                ParameterCollection("IL-2", [c0(22e-12)], field_quantity="il2"),
                ParameterCollection("IL-2", [sec_sigma(0.0)], field_quantity="il2")
