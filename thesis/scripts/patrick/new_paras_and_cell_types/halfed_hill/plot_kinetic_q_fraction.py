...
path = "/extra/brunner/thesis/kinetic/test_v2/"
...
global_df["time"] = global_df["time"].mul(0.5)
# cell_df["time"] = cell_df["time"].mul(10)
R_mean_plotting["time"] = R_mean_plotting["time"].mul(0.5)
...
ax.set(xlabel="t", ylabel="each cells R", title="gamma = " + str(cell_df_1.loc[cell_df["scan_index"]==0, "IL-2_gamma"].values[0])) #, ylim=(0.275,0.45))
plt.show()

fig = plt.figure()
ax = sns.lineplot(x="time", y="IL-2_R", data=cell_df_1.loc[(cell_df["scan_index"]==1)], estimator=None, units="id", legend=False)
ax.set(xlabel="t", ylabel="each cells R", title="gamma = " + str(cell_df_1.loc[cell_df["scan_index"]==1, "IL-2_gamma"].values[0])) #, ylim=(0.275,0.45))
plt.show()
# del fig, ax
# fig = plt.figure()
