...
# gammas = sorted(global_df["IL-2_gamma"].unique())[10:]
...
for gammas in gammas_list:
    for frac in [0.25]:
        ...
        #              "scan_index": a_cell_df["scan_index"].unique()[0], my_hue: my_hue_value}, ignore_index=True) #, "pSTAT5": a_cell_df["pSTAT5_pSTAT5"].mean(), "EC50": a_cell_df["EC50_EC50"].mean()
        ...
        c_v_df = pd.DataFrame(index=range(len(averaging_values)), columns=["time"] + list(cell_df[my_hue].unique()))
        c_v_df["time"] = averaging_values
        for my_hue_value in cell_df[my_hue].unique(): #g
            ...
            for v, value in enumerate(averaging_values): #t
                ...
                std.append(np.std(tmp_values))
                mean.append(np.mean(tmp_values))
                c_v_over_t.append(np.std(tmp_values)/np.mean(tmp_values))
            c_v_df[my_hue_value] = c_v_over_t
            c_v_df["time_index"] = time_index
...
c_v_df["time"] = c_v_df["time"].div(3600)
...
ax.set(ylabel="fold change", xlabel="time (h)", yscale=yscale, xscale=xscale, ylim=ylim, xlim=xlim, title="$c_v$")
...
saving_string =r"/home/brunner/Documents/Current work/04122020/" + "kin_analytical_c_v_f_" + str(cell_df["IL-2_Tsec_fraction"].unique()[0]) + ".png"
