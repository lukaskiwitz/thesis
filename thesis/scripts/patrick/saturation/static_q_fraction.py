...
Kc = ScannablePhysicalParameter(t_Kc(0.01), lambda x, v: v)
...
for w in ["R_saturation"]:
    for v in np.linspace(0.00005,1.0,15): #np.logspace(-1,1,3):
        for Kc_value in [0.001,0.01,0.1,1,10,100,1000]:

            sim_parameters = [
                ParameterCollection("IL-2", [D(10)], field_quantity="il2"),
                ParameterCollection("IL-2", [fraction(v)], field_quantity="il2"),
                ParameterCollection("IL-2", [Kc(Kc_value)], field_quantity="il2")
                #ParameterCollection("IL-2", [run(w)], field_quantity="il2")
            ]
            """Scans over parameters that are associated with a field"""

            """Scans over parameters that are associated with an entity_type"""
            entity_types = [
                default.get_updated([ParameterCollection("IL-2", [bc_type(w)])]),
                changed.get_updated([ParameterCollection("IL-2", [bc_type(w)])]),

                # default.get_updated([ParameterCollection("IL-2", [Kc(Kc_factor)])]),
                # changed.get_updated([ParameterCollection("IL-2", [Kc(Kc_factor)])]),
