...
try:
    ...
    for scan_index in global_df["scan_index"].unique():
        for t in global_df["time"].unique():
            global_df.loc[(global_df["scan_index"] == scan_index) & (global_df["time"] == t), "surf_c_std"] = \
            cell_df.loc[(cell_df["scan_index"] == scan_index) & (cell_df["time"] == t), "IL-2_surf_c"].std()
...
my_hue = "IL-2_Kc"
x_axis = "IL-2_fraction"
averaging_values = np.sort(global_df[x_axis].unique())

# global_df = pd.concat((get_dataframes[x][0] for x in range(len(get_dataframes)))).groupby(["IL-2_D", x_axis], as_index=False).mean()
...
# global_df["IL-2_D"] /= ((10 ** 0.5 * 0.01) ** 2)
# global_df["k_factor"] = ["$%s$" % x for x in global_df["k_factor"]]


print("averaging receptor levels")
R_mean_df_list = []
for j, value in enumerate(averaging_values):
    a_cell_df = cell_df.loc[cell_df["scan_index"] == j]
    ...
    R_mean_df = R_mean_df.append(
        {x_axis: value, "R_mean": a_cell_df["IL-2_R"].mean(),"std": a_cell_df["IL-2_R"].std(),
         ...
         "scan_index": j}, ignore_index=True)
...
ax1  = sns.lineplot(x=x_axis, y="Concentration", data=global_df.sort_values(by=x_axis)[startingPoint:stoppingPoint],hue=my_hue, hue_order = hue_order)#,legend=False)
...
#sns.lineplot(x="fraction", y="mean_surf_c_il2", data=saving_dataframe,hue=group_variables[1])
ax3  = sns.lineplot(x=x_axis, y="surf_c", data=global_df.sort_values(by=x_axis)[startingPoint:stoppingPoint],hue=my_hue, hue_order = hue_order,legend=False)
...
ax2 = sns.lineplot(x=x_axis, y="Gradient", hue=my_hue, hue_order = hue_order, data=global_df.sort_values(by=x_axis)[startingPoint:stoppingPoint], legend=False)
ax2.set(xlabel="fraction", ylabel="mean gradient in nM/um", title="gradient", yscale=yscale, xscale=xscale)

fig.add_subplot(a_x,a_y, 7)
global_df["std_norm"] = global_df["SD"]/global_df["Concentration"]
ax5 = sns.lineplot(x=x_axis, y="std_norm", data=global_df.sort_values(by=x_axis)[startingPoint:stoppingPoint], hue=my_hue, hue_order = hue_order, label="std_norm", legend=False)
ax5.set(xlabel="", ylabel="normed std.", title="normed std", yscale=yscale, xscale=xscale, ylim=ylim, xticklabels=[])
ax5.set_xticklabels([])
# ax5.set_yticks(np.array([1e-2, 1e-1, 1e0,1e1]),minor=True)

fig.add_subplot(a_x,a_y, 4)
global_df["SD"] = global_df["SD"]
ax4 = sns.lineplot(x=x_axis, y="SD", data=global_df.sort_values(by=x_axis)[startingPoint:stoppingPoint], hue=my_hue, hue_order = hue_order, label="std", legend=False)
...
ax6 = sns.lineplot(x=x_axis, y="surf_c_std", data=global_df.sort_values(by=x_axis)[startingPoint:stoppingPoint], hue=my_hue, hue_order = hue_order, label="surf_c_std", legend=False)
...
ax7 = sns.lineplot(x=x_axis, y="surf_c_std_norm", data=global_df.sort_values(by=x_axis)[startingPoint:stoppingPoint], hue=my_hue, hue_order = hue_order, label="surf_c_std", legend=False)
ax7.set(xlabel="fraction", ylabel="surf.c. std./mean surf.c.", title="normed surf. std", yscale=yscale, xscale=xscale, ylim=ylim)
...
ax8.set(xlabel="", ylabel="Receptors", title="R mean", xticklabels=[], xscale="linear", ylim=ylimR)
ax8.set_xticklabels([])
...
ax9.set(xlabel="", ylabel="std. in Receptors", title="R std", xticklabels=[], xscale="linear", ylim=ylimR)
ax9.set_xticklabels([])
...
ax10.set(xlabel="fraction", ylabel="std./mean", title="normed R std", xscale="linear", ylim = ylimR)
plt.show()