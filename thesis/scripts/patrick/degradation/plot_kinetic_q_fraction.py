...
# path = "/extra/brunner/thesis/kinetic/q_fraction_medium_g_scan_multi/run1/"
path = "/extra/brunner/thesis/kinetic/q_fraction_medium_degradation_scan/"
...
# sns.lineplot(x=x_axis, y="surf_c_std_norm", data=global_df.loc[global_df["IL-2_gamma"] == '$10.0$'].sort_values(by="time")[startingPoint:stoppingPoint], color=palette[0])
# sns.lineplot(x=x_axis, y="surf_c_std_norm", data=global_df.loc[global_df["IL-2_gamma"] == '$2.0$'].sort_values(by="time")[startingPoint:stoppingPoint], color=palette[1])
# sns.lineplot(x=x_axis, y="surf_c_std_norm", data=global_df.loc[global_df["IL-2_gamma"] == '$0.5$'].sort_values(by="time")[startingPoint:stoppingPoint], color=palette[2])
# ax7 = sns.lineplot(x=x_axis, y="surf_c_std_norm", data=global_df.loc[global_df["IL-2_gamma"] == '$0.1$'].sort_values(by="time")[startingPoint:stoppingPoint], color=palette[3])
#
#
#
# ax7.set(xlabel="time in h", ylabel="c$_v$", yscale=yscale, xscale=xscale, ylim=ylim, xticks = [0, 50, 100,150,200])
# plt.hlines(y=global_df.loc[(global_df["time_index"] == 0), "surf_c_std_norm"][0],xmin=0,xmax=190)
...
# new_labels = ["negative feedback", "no feedback", "positive feedback"]
...
# plt.legend(handles=new_handles, labels=new_labels, loc='upper center', fancybox=True)
# fig.savefig("plots/" + "kin_q_c_v_fixed" + ".png", bbox_inches='tight')
# plt.show()
#
...
# plt.show()
sns.set(font_scale=1, rc={"lines.linewidth":1})
...
ax1.set(xlabel="", ylabel="mean c. in nM", title="concentration", yscale=yscale, xscale=xscale, ylim=ylim, xlim=xlim)#, xticklabels=[]) #, ylim=(0.275,0.45))
ax1.set_xticklabels([])
...
#ax1.errorbar(np.linspace(0.00005,1.0,20), plotting_df[5:]["surf_c"], yerr=plotting_df[5:]["sd"], fmt='-o')
ax3.set(xlabel="time in h", ylabel="mean surf.c. in nM", title="surface c.", yscale=yscale, xscale=xscale, ylim=ylim, xlim=xlim)#, xticklabels=[]) #, ylim=(0.275,0.45))
# plt.show()
ax3.set_xticklabels([])
...
ax2.set(xlabel="time", ylabel="mean gradient in nM/um", title="gradient", yscale=yscale, xscale=xscale, xlim=xlim)
...
ax5.set(xlabel="", ylabel="normed std.", title="normed std", yscale=yscale, xscale=xscale, ylim=(None,None), xticklabels=[], xlim=xlim)
ax5.set_xticklabels([])
...
ax4.set(xlabel="", ylabel="std. in nM", title="std", xticklabels=[], yscale=yscale, xscale=xscale, ylim=ylim, xlim=xlim)
ax4.set_xticklabels([])
...
ax6.set(xlabel="", ylabel="surf.c. std. in nM", title="surf. std", xticklabels=[], yscale=yscale, xscale=xscale, ylim=ylim, xlim=xlim)
ax6.set_xticklabels([])
#
#
fig.add_subplot(a_x,a_y, 8)
...
ax7.set(xlabel="time", ylabel="surf.c. std./mean surf.c.", title="normed surf. std", yscale=yscale, xscale=xscale, ylim=(None, None), xlim=xlim)
...
ax10 = sns.lineplot(x=x_axis, y="h1_norm", data=global_df.sort_values(by="time")[startingPoint:stoppingPoint], hue=my_hue, hue_order=hue_order, label="R_std", legend=False)
ax10.set(xlabel="time", ylabel="H1 norm", title="H1 norm", xscale="linear", yscale=yscaleR, ylim = ylimR)
plt.show()
...
fig = plt.figure()
ax = sns.lineplot(x="time", y="IL-2_R", data=cell_df_1.loc[cell_df["scan_index"]==0], estimator=None, units="id", legend=False)
ax.set(xlabel="t", ylabel="each cells R", title="gamma = " + str(cell_df_1.loc[cell_df["scan_index"]==0, "IL-2_gamma"][0])) #, ylim=(0.275,0.45))
plt.show()
