    ...
    if "std" in plot_variables:
        ...
        if len(plot_variables) == 1:
            ax2 = sns.lineplot(x=group_variables[0], y="std_norm",
                               data=plotting_df[data_start:data_end].loc[plotting_df[data_start:data_end]["IL-2_D"] == single_D], label="std", legend=False,
                               color="orange")
    ...
    if "concentration" in plot_variables:
        if len(plot_variables) == 1:
            ax3 = sns.lineplot(x=group_variables[0], y="Concentration",
                               data=plotting_df[data_start:data_end].loc[plotting_df[data_start:data_end]["IL-2_D"] == single_D], label="concentration",
        ...
        if len(plot_variables) == 1:
            ...
            if ax1:
                ...
                if group_variables[0] == "IL-2_sigma":
                    ...
                    ax1.figure.legend(reversed(handles), reversed(labels), loc=(0.262, 0.2))  # (loc=(0.385,0.2))
            elif ax2:
