...
def histogram_plotting(avg_hist_df, gamma, time, color, label, exp_fit=False):
    ...
    if exp_fit == True:
        ...
        plt.plot(func(myRange, *popt), color="black", lw=1)
        #
        #
        # def func(x,a,b,c):
        #     return a/x +0*b*c
...
for sv,scan_v in enumerate(scan_variables):
    ...
    if pSTAT5 == True:
        ...
        AC_1_2_niche, AC_1_2_autocorr = AC_1_2(pSTAT5_sec_histograms, gammas, timepoints, cell_cell_distance, max_distance, func, plot=False, linear=False, p0=None)
...
plt.plot(scan_variables, unavg_EC50_niche_over_sv, label="unavg. EC50 Threshold", color="red")
...
scan_variables = [scan_variables[x] for x in [1,10]]
...
for sv,scan_v in enumerate(scan_variables):
    if len(str(scan_v)) > 5:
        scan_v = round(scan_v,2)
    avg_hist_df = pd.read_hdf(base_path + str(no_of_runs) + '_runs_' + str(scan_v) + saving_string + '_avg_hist_df.h5', mode="r")
    histogram_plotting(avg_hist_df,gammas[0],float(timepoints[0]),palette[sv], label=scan_v, exp_fit=True)
...
plt.legend()
# plt.savefig("/home/brunner/Documents/Current work/16102020/niches_comparison.png", bbox_inches='tight')
plt.show()