...
# for v in np.logspace(-1,1,1):
    ...
    sim_parameters = [
        ...
        # ParameterCollection("numeric", [is_linear(v)]),
    ]

    """Scans over parameters that are associated with an entity_type"""
    entity_types = [
        default.get_updated([ParameterCollection("bc_type", [bc_type(v)])]),
        abs.get_updated([ParameterCollection("bc_type", [bc_type(v)])]),
        sec.get_updated([ParameterCollection("bc_type", [bc_type(v)])]),
    ]
