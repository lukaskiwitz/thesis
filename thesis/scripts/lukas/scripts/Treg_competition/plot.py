...
def steady_state_plot(fig, ax, df, time_name, x_name, y_name, x_label, y_label, legend="brief", hue="type_name",
                      leg_loc="upper right", style="scan_name_scan_name"):
    df = df.loc[df[time_name] == t_max]
    if style in df.columns:
        sns.lineplot(x=x_name, y=y_name, data=df, hue=hue, ax=ax, legend=legend, palette=color_dict,style= style)
    else:
...
def reset_scan_index(df):
    offsets = {}
    if not "scan_name_scan_name" in df.columns :
        return df
...
cell_df = reset_scan_index(cell_df)
...
#                       "t={t_max}. Cytokine ({c})".format(c=c_unit, t_max=t_max), hue= "field_name",style="scan_name_scan_name")
#     steady_state_plot(fig, ax[1][0], global_df, "time_index", "scan_index", "Gradient", scan_label,
#                       "t={t_max} Gradient ({c}/$\mu m$)".format(c=c_unit, t_max=t_max), hue= "field_name", style="scan_name_scan_name", legend=False)
#     steady_state_plot(fig, ax[1][1], global_df, "time_index", "scan_index", "SD", scan_label,
#                       "t={t_max} SD of Nodal Values".format(c=c_unit, t_max=t_max),  hue= "field_name",style="scan_name_scan_name", legend=False)
...
# from nearest_neighbour.model import my_nn, my_nn_plot
#
# def get_dist(type_1,type_2,df,r = 5000):
#
#
#     X,cells,con,dist,nn = my_nn(df,type_1)
#
#     cell1 = df.loc[df["type_name"] == type_1]
#
#     cell2 = df.loc[df["type_name"] == type_2]
#
#     type_2_distances = []
#
#     for i,cell in cell2.iterrows():
#
#         p = np.array([[cell["x"],cell["y"]]])
#         type_2_distances.append(nn.radius_neighbors(p,radius=r)[0][0])
#
#     type_2_distances = np.array(type_2_distances)
#
#     s = pd.Series(np.min(type_2_distances, axis=1))
#     s.index = cell2.index
#
#     cell2.insert(0,"min",s)
#
#     return type_2_distances, cell2
#
# cells_small = cell_df.loc[
#
#     (cell_df["scan_name_scan_name"] == "cluster_strength")
#
# ]
#
# for o,cells_small in cell_df.groupby(["scan_name_scan_name"]):
#
#     for i,df in cells_small.groupby(["scan_index"]):
#         try:
#             default_dist,defaults = get_dist("effector","default",df)
#             if "min" not in cell_df.columns:
#                 cell_df["min"] = defaults["min"]
#             else:
#                 cell_df["min"].update(defaults["min"])
#         except:
#             pass
#
#         # sns.scatterplot(x="x",y="y",data =defaults, hue = "min",s = 100)
#         # plt.show()
#
#
# #
...
# ]
#
# sns.lineplot(x="min",y="IL-2_surf_c",data = df,hue = "scan_index",ci=None)
# plt.xlabel("minimum distance from effector cell")
# plt.ylabel("IL-2 surface concentration (nM)")
# # plt.yscale("log")
# plt.show()
