...
class kineticSolver(InternalSolver):
    ...
    def step(self,t1,t2,dt,p,entity=None):
        ...
        try:
            ...
            gamma = float(p.get_misc_parameter("gamma", "misc").get_in_sim_unit())
        if gamma == 1:
            p.get_physical_parameter("R", "IL-2").set_in_sim_unit(initial_R)
            ...
            if celltype != "Tsec" and celltype != "Tnaive":
                ...
                if self.T[0] == t1: #set initial receptors
                    ...
                    try:
                        half_time = p.get_misc_parameter("pos_half_time", "misc").get_in_post_unit() if gamma > 1 else p.get_misc_parameter("neg_half_time", "misc").get_in_post_unit()
                    except:
                    ...
                    new_R, new_states = self.c_response_delayed(p, Km, N, eta, k_max  * k_max_factor, k_min, il2, gamma, t1, dt, states, half_time, EC50)
