...
class kineticSolver(InternalSolver):
    ...
    def c_response_delayed(self, p, Km, N, eta, k_max, k_min, il2, gamma, t1, dt, states, half_time, EC50,
                           pSTAT5_response=True):
        ...
        dict = {"N": N, "gamma": gamma, "eta": eta, "il2": il2, "feedback_k": feedback_k,
    ...
    def step(self, t1, t2, dt, p, entity=None):
        ...
        try:
            ...
            gamma = float(p.get_physical_parameter("gamma", "misc").get_in_sim_unit())
        if gamma == 1:
            p.get_physical_parameter("R", "IL-2").set_in_sim_unit(initial_R)
            ...
            if celltype != "Tsec" and celltype != "Tnaive":
                ...
                il2 = p.get_physical_parameter("surf_c",
                                               "IL-2").get_in_post_unit() * 1e-9  # post is nM, neeeded in units of Km --> M
                ...
                if self.T[0] == t1:  # set initial receptors
                    ...
                    states = p.get_misc_parameter("states", "misc")
                    pSTAT_k = p.get_misc_parameter("pSTAT_k", "misc").get_in_post_unit() if p.get_misc_parameter(
                        "pSTAT_k", "misc") is not None else 860
                    pSTAT_N = p.get_misc_parameter("pSTAT_N", "misc").get_in_post_unit() if p.get_misc_parameter(
                        "pSTAT_N", "misc") is not None else 1

                    EC50 = self.EC50_calculation(E_max=125e-12, E_min=0, k=pSTAT_k, N=int(pSTAT_N), R=R_il2)
