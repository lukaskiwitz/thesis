...
def test1(p_global):
    ...
    domain = Entity.DomainSphere([0,0,0],p_global["radius"],p_domain,[
           bc.outerIntegral(lambda u,p:0,"on_boundary",fieldQuantity="il2")
#           bc.outerDirichletBC(u_exact_1(p_global["radius"],p_cell),"on_boundary",fieldQuantity="il2")
            ])
    ...
    sc.path = path
    sc.initialize(max_char_length=0.1,min_char_length=0.0001)
...
#           bc.outerIntegral(lambda u,p: fcs.Constant(1),"true",fieldQuantity="il2"),
    ...
    sc.path = path
    sc.initialize(max_char_length=0.01,min_char_length=0.0001)
...
#    plt.plot(x_2,r_2,color="tab:orange")
    plt.legend(["FEM_low","exact solution","FEM_high","exact solution"])
