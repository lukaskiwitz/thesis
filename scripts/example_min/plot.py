...
if len(global_df["scan_index"].unique()) > 1:
    ...
    maximum_global_plot(fig,ax[1][0],global_df,"Gradient",scan_label,"max. Gradient ({c}/$\mu m$)".format(c=c_unit), legend=False)
...
"""Cluster Plot"""
fig, ax = plt.subplots(2, 2, sharex=True, sharey=False)
scan_score_plot(fig, ax[0][0], cell_df, "default_score_norm", r"default score t-1(normalized)", legend=False,ylim=[0,score_max])
scan_score_plot(fig, ax[1][0], cell_df, "sec_score_norm", r"sec score t-1 (normalized)", legend=False,ylim=[0,score_max])

scan_score_plot(fig, ax[0][1], cell_df, "default_score_init_norm", r"default score t=0 (normalized)", legend=False,ylim=[0,score_max])
scan_score_plot(fig, ax[1][1], cell_df, "sec_score_init_norm", r"sec score t=0 (normalized)", legend="full",ylim=[0,score_max])
