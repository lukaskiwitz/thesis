...
IMGPATH = "/home/{u}/dev_testing_images/".format(u=user)
...
global_plot(fig,ax[0][1], global_df,"maxGradient", "avg. Gradient ({c}/dm)".format(c=c_unit),legend=False)
...
# steady_state_plot(fig,ax[0][0],global_df,"time_index","scan_index","Concentration",scan_label,"t={t_max}. Cytokine ({c})".format(c=c_unit, t_max = t_max),hue="field_name")
# maximum_global_plot(fig,ax[1][0],global_df,"Gradient",scan_label,"max. Gradient ({c}/dm)".format(c=c_unit), legend=False)
# maximum_global_plot(fig,ax[1][1],global_df,"SD",scan_label,"max. SD of Nodal Values".format(c=c_unit), legend=False )
# steady_state_plot(fig,ax[0][1],counts,"time","scan_index","n",scan_label,"t={t_max} Number of cells".format(t_max = t_max),leg_loc="upper left")
#
# plt.tight_layout()
...
"""Cluster Plot"""
fig, ax = plt.subplots(2, 2, sharex=True, sharey=False)
scan_score_plot(fig, ax[0][0], cell_df, "Tfh_score_norm", r"Tf$_h$ score (normalized)", legend=False,ylim=[0,score_max])
scan_score_plot(fig, ax[1][0], cell_df, "Th1_score_norm", r"Th$_1$ score (normalized)", legend=False,ylim=[0,score_max])
scan_score_plot(fig, ax[0][1], cell_df, "Tn_score_norm", r"T$_n$ score (normalized)", legend="brief",ylim=[0,score_max])
...
fig, ax = plt.subplots(2, 2, sharex=True, sharey=True)
sns.lineplot(x = "time_index", y = "IL-2_surf_c", hue = "type_name", data=cell_df ,ax=ax[0][0])
sns.lineplot(x = "time_index", y = "IL-6_surf_c", hue = "type_name", data=cell_df ,ax=ax[0][1])
sns.lineplot(x = "time_index", y = "IFNg_surf_c", hue = "type_name", data=cell_df ,ax=ax[1][0])
plt.show()
